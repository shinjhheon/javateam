...
<div class = "container-fluid">
		<div class="row">
			<table class="table table=striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					<%
						jsBbsDAO jsbbsDAO = new jsBbsDAO(); // 객체 생성
						ArrayList<jsBbs> list = jsbbsDAO.getList(pageNumber);
						for(int i = 0; i < list.size(); i++){
					%>
					<tr>
						<td><%= list.get(i).getJsbbsID() %></td>
						<!--  게시글 제목을 누르면 해당 글을 볼 수 있도록 링크를 건다. -->
						<td><a href = "view.jsp?bbsID=<%= list.get(i).getJsbbsID() %>">
							<%= list.get(i).getJsbbsTitle().replaceAll(" ", "&nbsp;")
									.replaceAll("<", "&lt;").replaceAll(">","&gt;").replaceAll("\n","<br>") %></a></td>
						<td><%= list.get(i).getUserID() %></td>
						<td><%= list.get(i).getJsbbsDate().substring(0,11) + list.get(i).getJsbbsDate().substring(11,13) + "시" + list.get(i).getJsbbsDate().substring(14,16) + "분 " %></td>
					</tr>
					<%
					}
					%>
			...
			<% 
				if(jsbbsDAO.nextPage(pageNumber + 1)){
			%>
				<a href="jascbbs.jsp?pageNumber=<%=pageNumber + 1%>"
		...
		<a href="jcwrite.jsp" class="btn btn-dark pull-right">글쓰기</a>
	</div>	
	<!-- 게시판 메인 페이지 영역 끝 -->
				</main></div>
</div>
